...
public class CollectionLogPlugin extends Plugin
{
	...
	{
		if (event.getOption().equals(COLLECTION_LOG_EXPORT) && event.getTarget().endsWith(COLLECTION_LOG_TARGET))
		{
			exportItems();
		}
	}
	...
	private void exportItems()
	{
		...
		try (JsonWriter writer = new JsonWriter(new FileWriter(filePath, StandardCharsets.UTF_8)))
		{
			writer.setIndent("  ");
			writer.beginObject();
	...
	private void loadItems()
	{
		...
		completedCategories = GSON.fromJson(completed, new TypeToken<List<String>>()
		{
		}.getType());
	}
