...
public class CrowdsourcingWoodcutting
{
	...
	{
		MenuOpcode menuAction = menuOptionClicked.getMenuOpcode();
		int id = menuOptionClicked.getIdentifier();
		if (state == SkillingState.READY && menuAction == MenuOpcode.GAME_OBJECT_FIRST_OPTION && TREE_OBJECTS.contains(id))
		{
			state = SkillingState.CLICKED;
			lastExperimentEnd = client.getTickCount();
			treeId = id;
			treeLocation = WorldPoint.fromScene(client, menuOptionClicked.getParam0(), menuOptionClicked.getParam0(), client.getPlane());
		}
		else
		{
			endExperiment(SkillingEndReason.INTERRUPTED);
