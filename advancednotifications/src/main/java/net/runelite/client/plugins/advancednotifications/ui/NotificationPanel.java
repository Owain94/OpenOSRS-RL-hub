...
public abstract class NotificationPanel<N extends Notification> extends JPanel
{
	protected static class DragStarter extends MouseAdapter
	{
		...
		{
			if (parent.getNotifications().contains(child))
			{
				return true;
			}
			...
			{
				if (n instanceof DraggableContainer && containerContains((DraggableContainer) n, child))
				{
					return true;
				}
			}
	...
	public static NotificationPanel<?> buildPanel(DraggableContainer container, Notification notif)
	{
		...
		{
			return new ItemNotificationPanel((ItemNotification) notif, container);
		}
		if (notif instanceof EmptyNotification)
		{
			return new EmptyNotificationPanel((EmptyNotification) notif, container);
		}
		if (notif instanceof NotificationGroup)
		{
			return new NotificationGroupPanel((NotificationGroup) notif, container);
		}
